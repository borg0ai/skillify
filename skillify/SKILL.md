---
name: skillify
description: Use when creating, reviewing, packaging, or publishing reusable Agent Skills for npx skills, especially when a repository needs valid SKILL.md frontmatter, discoverable layout, focused references, or release-safe contents.
license: MIT
metadata:
  version: "0.1"
---
...
For multiple skills, use `skills/<skill-name>/SKILL.md`. Keep each skill's references, scripts, and assets inside its own directory. Read [repository-layout.md](references/repository-layout.md) when choosing between layouts.
...
3. Write frontmatter with exactly useful `name` and `description` fields. Start description with `Use when...`; describe triggers, not workflow summary.
4. Put activation signals first. Explain when to use and when not to use.
5. Write short ordered actions. Each action must be executable without hidden assumptions.
6. Keep small patterns inline. Move heavy reference material to `references/`; move reusable executable logic to `scripts/`.
7. Preserve user intent, existing project conventions, and safe failure behavior. Never invent files, commands, APIs, or verification results.
8. Remove every test, fixture, build artifact, dependency directory, secret, and unrelated document from the release directory.

## Validation gate

Before release, inspect the package as an installer would:
...
- skill name matches selected install name;
...
- Workflow in description: agent skips body. Keep description trigger-only.
